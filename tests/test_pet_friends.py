...
class TestAuth:
    ...
    @allure.title("Получение API ключа")
    @allure.severity(Severity.BLOCKER)
    @pytest.mark.smoke
    def test_get_api_key_for_valid_user(self, email=valid_login, password=valid_password):
...
class TestPets:
    ...
    @allure.title("Получение списка питомцев с фильтром: {filter}")
    ...
    @pytest.mark.parametrize("filter", (
        ...
        special_chars(),
        "12345"
    ), ids=("250 symbols", "1001 symbols", "russian", "RUSSIAN", "chinese", "special symbols", "digit"))
    def test_get_all_pets_list_with_valid_data(self, get_data, filter):
        status, result = pf.get_list_pets(get_data["auth_key"], filter)
        assert status == 200
        assert len(result) > 0

    @allure.title("Добавление питомца с фото")
    @allure.severity(Severity.CRITICAL)
    @pytest.mark.smoke
    def test_add_new_pet_with_valid_data(self, get_data):
    ...
    @allure.title("Добавление питомца без фото [name={name}, type={animal_type}, age={age}]")
    @allure.severity(Severity.NORMAL)
    ...
    def test_add_new_pet_without_photo(self, get_data, name, animal_type, age):
    ...
    @allure.title("Добавление фото существующему питомцу")
    @allure.severity(Severity.NORMAL)
    def test_add_photo_for_pet(self, create_yield_delete_test_pet):
        pet_photo = os.path.join(os.path.dirname(__file__), "images/Dog.jpg")
    ...
    @allure.severity(Severity.NORMAL)
    @pytest.mark.xfail
    def test_update_pet_info(self, create_yield_delete_test_pet):
    ...
    @allure.title("Удаление своего питомца")
    @allure.severity(Severity.CRITICAL)
    @pytest.mark.smoke
    def test_delete_pet(self, create_yield_delete_test_pet):
    ...
    @allure.title("Удаление чужого питомца")
    @allure.severity(Severity.NORMAL)
    @pytest.mark.negative
    def test_delete_of_another_pet(self, get_data):
